...
insert into value_tab_Settlement_Helper
	...
	where
		code in (select code from conversion.value_settlements);
go
...
insert into [sma_MST_SettlementType]
	...
	--	'Paid To Client'
	where
		vc.code in (select code from conversion.value_settlements)
	except
	select
		SettlTypeName
...
insert into [sma_TRN_Settlements]
	...
	select
		...
		case
			when v.code in ('SETTLEMENT', 'DEBTPAY', 'MEDPAY') then v.total_value
		end				as stlnSetAmt,					-- Gross Settlement
		null			as stlnNet,
		case
			when v.code in ('CLIENTADV') then v.total_value
		...
		case
			when v.code in ('ATTYFEE') then v.total_value
		end				as stlngrossattorneyfee,		-- Gross Fee
		case
			when v.code in ('REFERRAL') then v.total_value
	...
	where
		v.code in (select code from conversion.value_settlements)
go
