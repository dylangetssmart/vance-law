...
insert into value_tab_MedicalProvider_Helper
	...
	where
		code in (select code from conversion.value_medicalProviders)
go
...
insert into [sma_TRN_SpDamages]
	...
	select
		...
		case
			when V.[start_date] between '1900-01-01' and '2079-06-06' then CONVERT(DATE, V.[start_date])
			else null
		end																 as spddDateFrom,
		case
			when V.[stop_date] between '1900-01-01' and '2079-06-06' then CONVERT(DATE, V.[stop_date])
			else null
		end																 as spddDateTo,
...
-- Update medical provider start and end dates from the associated bills
-- Aggregate medical bills (spDamages) by hospital (recordId) and get the oldest date From and mst recent Date To
update h
set h.hosdStartDt = d.OldestDateFrom,
	h.hosdEndDt = d.MostRecentDateTo
from sma_TRN_Hospitals as h
inner join (
 select
	 spdnRecordID,
	 MIN(spddDateFrom) as OldestDateFrom,
	 MAX(spddDateTo)   as MostRecentDateTo
 from sma_TRN_SpDamages
 where spdsRefTable = 'Hospitals'
 group by spdnRecordID
) as d
	on d.spdnRecordID = h.hosnHospitalID;
