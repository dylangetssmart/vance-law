...
select 
	...
	case
	   when exists (select repeat_days from Skolrood_Needles.[dbo].[checklist_dir] where lim='Y' and matcode=A.cstsCode) 
		  then ( select max( floor(( repeat_days + 360 ) / 365 )) FROM Skolrood_Needles.[dbo].[checklist_dir] where lim='Y' and matcode=A.cstsCode)
	   else 
		  (select 1)
	end				   as [sldnYears],
	0				   as [sldnMonths],
